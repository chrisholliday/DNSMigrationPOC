...
<#
.SYNOPSIS
Sets up VNet peering between Hub and On-Prem VNets.
...
This script is separate from the deployment script to maintain clean separation
of concerns. Run this after phase1-3-deploy.ps1 completes successfully.
...
.EXAMPLE
./phase1-3-setup-peering.ps1 -Force

.EXAMPLE
./phase1-3-setup-peering.ps1 -OnpremResourceGroupName "my-onprem-rg" -HubResourceGroupName "my-hub-rg"
#>
...
Write-Host '║  Phase 1.3 - VNet Peering Setup                           ║' -ForegroundColor Cyan
Write-Host '╚════════════════════════════════════════════════════════════╝' -ForegroundColor Cyan
Write-Host ''
...
if ($onpremExists -eq 'false') {
    Write-Error "On-prem resource group '$OnpremResourceGroupName' not found"
}
...
if ($hubExists -eq 'false') {
    Write-Error "Hub resource group '$HubResourceGroupName' not found"
}
...
Write-Host "✓ On-prem VNet ID: $onpremVnetId" -ForegroundColor Green
...
$existingPeering = az network vnet peering show `
    --resource-group $HubResourceGroupName `
    --vnet-name $HubVnetName `
    --name 'hub-to-onprem' 2>$null | ConvertFrom-Json

if ($existingPeering) {
    Write-Host "  ℹ Peering 'hub-to-onprem' already exists (state: $($existingPeering.peeringState))" -ForegroundColor Gray
    
    if ($existingPeering.peeringState -eq 'Connected') {
        Write-Host '  ✓ Peering is already connected' -ForegroundColor Green
    }
    else {
        Write-Host "  ⚠ Peering exists but not connected (state: $($existingPeering.peeringState))" -ForegroundColor Yellow
    }
}
else {
...
if ($existingPeering) {
    Write-Host "  ℹ Peering 'onprem-to-hub' already exists (state: $($existingPeering.peeringState))" -ForegroundColor Gray
...
$hubPeering = az network vnet peering show `
    ...
    --vnet-name $HubVnetName `
    --name 'hub-to-onprem' `
    --query '{name:name, state:peeringState, allowForwarding:allowForwardedTraffic}' -o json | ConvertFrom-Json
...
if ($hubPeering.state -eq 'Connected' -and $onpremPeering.state -eq 'Connected') {
    Write-Host '✓ VNet peering configured successfully! Both peerings are connected.' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '  2. Proceed to Phase 1.4 to configure Hub DNS services' -ForegroundColor White
    exit 0
}
else {
    Write-Host '⚠ Warning: Peerings exist but may not be fully connected' -ForegroundColor Yellow
    Write-Host '  Check Azure Portal for peering status details' -ForegroundColor Gray
    exit 1
}
